...
RUN apk add --no-cache yq && \
  ...
  case $(uname -m) in \
  x86_64) \
  echo https://github.com/Radarr/Radarr/releases/download/v$VERSION/Radarr.master.$VERSION.linux-musl-core-x64.tar.gz > /tmp/download_url \
  ;; \
  aarch64) \
  echo https://github.com/Radarr/Radarr/releases/download/v$VERSION/Radarr.master.$VERSION.linux-musl-core-arm64.tar.gz > /tmp/download_url \
  ;; \
  *) \
